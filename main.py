...
TASK_REWARD          = 0.30   # single source of truth for task reward
...
def tasks_keyboard():
    kb = ReplyKeyboardMarkup(resize_keyboard=True)
    kb.row(KeyboardButton(f"📧 Create Account - Earn {TASK_REWARD:.2f}$"))
...
def handle_text(message):
    ...
    # ── Task Flow ──────────────────────────────────────────────────────────────
    elif text == f"📧 Create Account - Earn {TASK_REWARD:.2f}$":
        clear_state(uid)
        ...
        if account:
            ...
            add_balance(uid, TASK_REWARD)   # ← FIX: reward was never credited
